...
Paste the following T-SQL code, and replace the `SECRET` placeholder with the SAS token provided for the lab. Then execute the code to create the necessary credentials and external data source:  
...
Now we can retrieve the raw JSON file from Blob Storage and processes it into the relational tables. We use `OPENROWSET` with the `BULK` provider to read the `Recipes.json` file and store it in a variable. Then, we use `OPENJSON` to parse the JSON structure and populate the `Recipe` table.  
...
At this stage, the `Recipe` table is populated with one row per recipe. However, each recipe contains multiple ingredients, instructions, meal types, and tags, which must also be stored in their respective tables.  
...
You have successfully imported 50 recipes into the database, with all related data stored in their respective tables. 
...
Now let's add AI capabilities to our otherwise traditional relational database.
...
### Create the Recipe Vector Search Procedure
...
### Run Sample Queries and Observe Results
...
This stored procedure is **almost identical** to the previous `RecipeVectorSearch` procedure, but with one key difference:  
...
Now, let's **run the procedure** with a sample question:  
