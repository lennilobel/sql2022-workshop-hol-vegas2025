...
- The **vector model** understands **semantic relationships** rather than just exact keyword matches. For example, queries like `"Pineapple in the ingredients"` will also return recipes with mango in the ingredients as being only slightly less similar than pineapple.
