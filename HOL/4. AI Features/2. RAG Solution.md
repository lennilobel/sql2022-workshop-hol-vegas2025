...
This lab is based on the previous lab. You must have created your personal **AIDB** database and created the **VectorizeText** stored procedure as explained in the previous lab before proceeding.
...
The following code retrieves the raw JSON file from Blob Storage and processes it into the relational tables. We use `OPENROWSET` with the `BULK` provider to read the `Recipes.json` file and store it in a variable. Then, we use `OPENJSON` to parse the JSON structure and populate the `Recipe` table.  
...
### **Step 3: Verify the Vector Column**  
...
### **Step 2: Run Sample Queries and Observe Results**  
