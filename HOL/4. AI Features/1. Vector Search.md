...
A **vector** is a list of numbers (typically floating point numbers) generated from a deeply trained AI **text embedding model** that represents the **semantic meaning** of a piece of text. Vectors can then be compared for similarity or distance. For example, notice how the close together the vector values of "The Godfather" and "Crime Drama" are, indicating similarity:
