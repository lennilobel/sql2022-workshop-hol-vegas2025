...
1. In the **SQL Server Management Studio (SSMS)** Object Explorer, click on **Connect** → **Database Engine**.
...
In the next step, you will be using **OpenAI's text-embedding-3-large model**, which **returns 3072-dimensional vectors** (that is, where each vector contains 3072 floating point numbers). However, **SQL Server currently only supports vectors up to 1998 dimensions**.
...
- **Receive a vector** representation (an array of floating point nubmers) that captures the semantic meaning of the input text.

Run the following T-SQL script to create the `VectorizeText` stored procedure:
...
After pasting in the above code, change the placeholder for the API key (on the `DECLARE @OpenAIApi key` line) to the value supplied by the lab instructor.

### How it Works

This stored procedure accepts any text, and uses ``sp_invoke_external_rest_endpoint`` to call Azure OpenAI. It passes in the text, receives a vector back from Azure OpenAI, and returns the result via an output parameter declared as the `vector(1536)` data type.
...
1. Finally, the raw JSON vector from the response is converted to a native `vector` data type in SQL Server, sized at 1536 dimensions (to match the number of dimensions returned by the `text-embedding-3-large` model, after being compressed from 3072 dimensions down to 1536). This value is returned in the stored procedure's `@Vector` output parameter.
...
## Step 5: Vectorize the Movie Titles
...
Run the following T-SQL script to create the `VectorSearch` stored procedure:
